...
const CHART_BASE_OPTIONS = { responsive: true, maintainAspectRatio: false, animation: { duration: 700, easing: 'easeOutCubic' }, interaction: { mode: 'nearest', intersect: false }, plugins: { tooltip: { enabled: true } } };
...
function drawCharts(){
  ...
  if(monthsEl){
    ...
    new Chart(monthsEl, {type:'line', data:{labels:monthLabels,
      ...
      options: Object.assign({}, CHART_BASE_OPTIONS, { plugins:{legend:{display:false}}, scales:{y:{beginAtZero:true,ticks:{precision:0},grid:{color:'#F1F5F9'}},x:{grid:{display:false}}} }) });
  }
  ...
  if(ratioEl){
    ...
    new Chart(ratioEl,{type:'doughnut',data:{labels:Object.keys(counts),datasets:[{data: hasData ? Object.values(counts) : [1,0,0],backgroundColor:['#059669','#7C3AED','#DC2626'],borderWidth:0}]},
      options: Object.assign({}, CHART_BASE_OPTIONS, { plugins:{legend:{position:'bottom',labels:{boxWidth:10,font:{size:11}}}}, cutout:'65%'} )});
  }
  ...
  if(bonusEl){
    ...
    new Chart(bonusEl,{type:'bar',data:{labels:monthLabels,datasets:[{label:'Bonus (₹k)',data:months.map(m=>buckets[m.key]),backgroundColor:'#0891B2',borderRadius:6}]},
      options: Object.assign({}, CHART_BASE_OPTIONS, { plugins:{legend:{display:false}}, scales:{y:{beginAtZero:true,grid:{color:'#F1F5F9'}},x:{grid:{display:false}}} }) });
  }
  ...
  if(deptEl){
    ...
    new Chart(deptEl,{type:'bar',data:{labels: labels.length?labels:['No data yet'],datasets:[{label:'Referrals',data: values.length?values:[0],backgroundColor:chartColors(),borderRadius:6}]},
      options: Object.assign({}, CHART_BASE_OPTIONS, { indexAxis:'y', plugins:{legend:{display:false}}, scales:{x:{beginAtZero:true,ticks:{precision:0},grid:{color:'#F1F5F9'}},y:{grid:{display:false}}} }) });
  }
  // HR-only charts
  ...
  if(funnelEl){
    ...
    new Chart(funnelEl,{type:'bar',data:{labels:stages,datasets:[{label:'Candidates',data:stages.map(s=>state.referrals.filter(r=>r.status===s).length),backgroundColor:stages.map(s=>statusColor(s)),borderRadius:6}]},
      options: Object.assign({}, CHART_BASE_OPTIONS, { plugins:{legend:{display:false}}, scales:{y:{beginAtZero:true,ticks:{precision:0},grid:{color:'#F1F5F9'}},x:{grid:{display:false},ticks:{font:{size:10}}}} }) });
  }
  ...
  if(sourceEl){
    ...
    new Chart(sourceEl,{type:'pie',data:{labels: labels.length?labels:['No referrals yet'],datasets:[{data: values.length?values:[1],backgroundColor:['#2563EB','#0891B2','#DC2626','#D97706','#059669','#7C3AED'],borderWidth:0}]},
      options: Object.assign({}, CHART_BASE_OPTIONS, { plugins:{legend:{position:'bottom',labels:{boxWidth:10,font:{size:11}}}} }) });
  }
  ...
  if(deptHrEl){
    ...
    new Chart(deptHrEl,{type:'doughnut',data:{labels: labels.length?labels:['No data yet'],datasets:[{data: values.length?values:[1],backgroundColor:chartColors(),borderWidth:0}]},
      options: Object.assign({}, CHART_BASE_OPTIONS, { plugins:{legend:{position:'bottom',labels:{boxWidth:10,font:{size:11}}}}, cutout:'60%'} ) });
  }
}
...
function hrDashboard(){
  ...
  <div class="fade-up">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:16px;margin-bottom:22px;">
      ...
      ${statCard('Days Since Referral (Joined, avg)', avgPipelineDays===null?'—':avgPipelineDays+'d', 'days', GRADS[7])}
    </div>
