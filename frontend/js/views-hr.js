    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:16px;">
      ...
      <div class="glass job-card" style="padding:20px;">
        ...
        <div style="display:flex;gap:8px;">
          <button class="btn btn-outline" style="flex:1;" data-edit-job="${j.id}">${icon('pencil',13)} Edit</button>
          <button class="btn btn-danger-ghost" data-delete-job="${j.id}">${icon('trash',13)} Delete</button>
        </div>
      </div>`}).join('')}
    </div>`}
  </div>`;
        ...
        <tbody>
          ${filtered.map(r=>{
            ...
            return `<tr style="${isRejected?'opacity:0.85;':''}">
              ...
              <td><strong>${r.candidateName}</strong>${r.fraudFlags&&r.fraudFlags.length?` <span title="Fraud flags present" style="color:var(--warning);vertical-align:middle;">${icon('warning',13)}</span>`:''}
                ${r.autoRejected?`<div style="font-size:10.5px;color:#DC2626;margin-top:2px;display:flex;align-items:center;gap:4px;">${icon('zap',11)} Auto-rejected · orig ${r.originalMatch||'—'}%${r.evaluationHistory&&r.evaluationHistory.length?` · re-eval ${r.matchPercent||'—'}%`:''}</div>`:''}
  ...
  <div class="modal-overlay" id="hrModalOverlay">
    <div class="glass-strong pop-in" style="max-width:640px;width:100%;padding:28px;max-height:88vh;overflow-y:auto;">
      <div style="display:flex;justify-content:space-between;">
        ...
        <button class="btn btn-ghost" id="closeHrModal" style="padding:6px;">${icon('x',16)}</button>
      </div>
      ...
      <div style="padding:12px 14px;border-radius:12px;background:rgba(220,38,38,0.07);border:1px solid rgba(220,38,38,0.18);margin-bottom:14px;">
        <div style="font-weight:700;font-size:12.5px;color:var(--coral);margin-bottom:8px;display:flex;align-items:center;gap:6px;">${r.autoRejected?icon('zap',13):icon('x',13)} ${r.autoRejected?'AI Auto-Rejected Candidate':'Rejected Candidate'}</div>
        ...
        <button class="btn btn-outline" style="font-size:12px;padding:6px 12px;color:#059669;margin-top:10px;" id="reopenBtn" data-reopen="${r.id}">${icon('refresh',12)} Reopen Candidate Profile</button>
      </div>`:''}
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;">
        ...
        <button class="btn btn-outline" style="font-size:12px;padding:6px 12px;" id="reanalyzeBtn" data-reanalyze="${r.id}">${icon('refresh',12)} Re-evaluate Resume</button>
        <button class="btn btn-outline" style="font-size:12px;padding:6px 12px;color:var(--primary);" id="overrideBtn" data-override="${r.id}">${icon('sliders',12)} Override AI Decision</button>
      ...
      <div style="padding:12px 14px;border-radius:12px;background:rgba(37,99,235,0.08);margin-bottom:14px;display:flex;justify-content:space-between;">
        <span style="font-size:13px;font-weight:700;display:inline-flex;align-items:center;gap:6px;">${icon('target',14)} Interview Prediction</span>
      ...
      ${r.strengths&&r.strengths.length?`<div style="margin-bottom:10px;font-size:13px;font-weight:700;display:flex;align-items:center;gap:6px;">${icon('zap',14)} Strengths</div><div style="font-size:12.5px;color:var(--ink-soft);margin-bottom:14px;">${r.strengths.join(' · ')}</div>`:''}
      ${r.weaknesses&&r.weaknesses.length?`<div style="margin-bottom:10px;font-size:13px;font-weight:700;display:flex;align-items:center;gap:6px;">${icon('warning',14)} Areas for Improvement</div><div style="font-size:12.5px;color:var(--ink-soft);margin-bottom:14px;">${r.weaknesses.join(' · ')}</div>`:''}
      ...
      <div style="margin-bottom:6px;font-size:13px;font-weight:700;display:flex;align-items:center;gap:6px;">${icon('shield',14)} Fraud / Risk Check</div>
      ...
      <div style="margin-bottom:6px;font-size:13px;font-weight:700;display:flex;align-items:center;gap:6px;">${icon('clock',14)} AI Decision / Override History</div>
...
function bindAllReferrals(){
  ...
  document.querySelectorAll('[data-reanalyze]').forEach(el=>{
    el.addEventListener('click', async ()=>{
      ...
      if(btn){ btn.disabled=false; btn.innerHTML=`<span style="display:inline-flex;align-items:center;gap:6px;">${icon('refresh',13)} Re-evaluate Resume</span>`; }
    });
  });
  ...
  <div class="modal-overlay" id="overrideOverlay">
    <div class="glass-strong pop-in" style="max-width:480px;width:100%;padding:26px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
        ...
        <button class="btn btn-ghost" onclick="document.getElementById('overrideOverlay').remove()" style="padding:6px;">${icon('x',16)}</button>
      </div>
            ...
            ${rows.map((r,i)=>{
              ...
              return `<tr style="opacity:${aboveThreshold?'1':'0.55'};">
                ...
                <td><div style="display:flex;gap:4px;flex-wrap:wrap;">
                  ${(r.decision.verdict==='recommended'||r.decision.verdict==='highly_recommended') ? `<button class="btn btn-primary" style="font-size:11px;padding:4px 10px;display:inline-flex;align-items:center;gap:4px;" data-sl-shortlist='${r.id}'>${icon('check',12)} Shortlist</button>` : ''}
                  ${r.decision.verdict==='rejected' ? `<button class="btn" style="font-size:11px;padding:4px 10px;background:rgba(220,38,38,0.12);color:var(--coral);display:inline-flex;align-items:center;gap:4px;" data-sl-reject='${r.id}'>${icon('x',12)} Reject</button>` : ''}
...
async function openPassportModal(r){
  ...
  <div class="modal-overlay" id="passportOverlay">
    <div class="glass-strong pop-in" style="max-width:680px;width:100%;padding:28px;max-height:88vh;overflow-y:auto;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        ...
        <button class="btn btn-ghost" onclick="document.getElementById('passportOverlay').remove()" style="padding:6px;">${icon('x',16)}</button>
      </div>
  ...
  try{
    ...
    body.innerHTML = `
      ...
      ${passportSection(icon('shield',14)+' Red Flags', (p.redFlags||[]).length? `<ul style="margin:0;padding-left:18px;color:var(--coral);">${p.redFlags.map(f=>`<li style="margin-bottom:6px;">${f}</li>`).join('')}</ul>` : '<div style="color:var(--ink-soft);font-size:12.5px;">No risk flags detected by AI screening.</div>')}
      ${passportSection(icon('users',14)+' Duplicate Check', dup.duplicate
      ...
      ${passportSection(icon('calendar',14)+' Employment Gaps', (p.employmentGaps||[]).length
      ...
      ${passportSection(icon('calendar',14)+' Date Integrity', (p.dateConflicts||[]).length
      ...
      ${passportSection(icon('briefcase',14)+' Credibility', (p.credibilityFlags||[]).length
  ...
  <div class="fade-up">
    ...
    <div class="glass" style="padding:18px;margin-bottom:16px;">
      ...
      <div id="bulkDropzone" class="bulk-dropzone" style="margin-top:14px;">
        <div style="color:var(--primary);margin-bottom:8px;display:flex;justify-content:center;">${icon('fileText',30)}</div>
...
function renderBulkFileList(){
  ...
  listEl.innerHTML = `<div style="display:flex;flex-wrap:wrap;gap:8px;">${bulkFiles.map((f,i)=>`
    <div class="glass" style="padding:8px 12px;display:flex;align-items:center;gap:8px;font-size:12px;">
      <span>${icon('fileText',14)}</span><span style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${f.name}</span>
      <span style="color:var(--ink-soft);font-size:11px;">${(f.size/1024).toFixed(0)} KB</span>
      <button class="btn btn-ghost" style="padding:2px 4px;color:var(--coral);" data-bulk-remove="${i}">${icon('x',14)}</button>
    </div>`).join('')}</div>`;
          ...
          <tbody>
            ${res.results.map(r=>{
              ...
              return `<tr>
                ...
                <td>${r.duplicate?`<span style="color:#D97706;font-size:12px;display:inline-flex;align-items:center;gap:4px;">${icon('warning',12)} ${r.duplicateOf||''}</span>`:'<span style="color:var(--ink-soft);font-size:12px;">No</span>'}</td>
                <td style="font-size:12px;">${r.success?`<span style="color:#059669;display:inline-flex;align-items:center;gap:4px;">${icon('check',12)} Success</span>`:r.duplicate?`<span style="color:#D97706;display:inline-flex;align-items:center;gap:4px;">${icon('minus',12)} Skipped</span>`:`<span style="color:#DC2626;display:inline-flex;align-items:center;gap:4px;">${icon('x',12)} Failed</span>`}</td>
