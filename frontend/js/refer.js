  ...
  <div class="modal-overlay" id="autofillModalOverlay">
    <div class="glass-strong pop-in" style="max-width:560px;width:100%;padding:26px;">
      <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:14px;">
        ...
        <button class="btn btn-ghost" id="closeAutofillModal" style="padding:4px 8px;">${icon('x',16)}</button>
      </div>
...
function bindReferView(){
  ...
  if(fileInput) fileInput.addEventListener('change', async ()=>{
    ...
    dropzone.innerHTML = `<span style="display:inline-flex;align-items:center;gap:6px;">${icon('loader',14)} Uploading and extracting text from <strong style="color:var(--ink);">${f.name}</strong>… (OCR may take a moment)</span>`;
    try{
      const result = await uploadResumeFile(f);
      dropzone.innerHTML = `<span style="display:inline-flex;align-items:center;gap:6px;">${icon('paperclip',14)} <strong style="color:var(--ink);">${result.fileName}</strong> attached — text extracted ${icon('check',14)}</span>`;
    ...
    if(dupEl){
      if(dupCheck && dupCheck.duplicate){
        dupEl.innerHTML = `<div class="glass pop-in" style="padding:14px 16px;margin-bottom:16px;border:1.5px solid rgba(220,38,38,0.4);display:flex;gap:12px;align-items:center;">
          <div style="color:var(--coral);display:flex;">${icon('alertTriangle',20)}</div>
